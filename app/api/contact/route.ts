...
export async function POST(req: NextRequest) {
  console.log("=== [SERVER LOG] CONTACT API ROUTE TRIGGERED ===");

  // 1. Fail-safe: Verify the API key exists inside the runtime environment
  if (!process.env.RESEND_API_KEY) {
    console.error("❌ CRITICAL CONFIG ERROR: RESEND_API_KEY is completely missing from your environment variables!");
    return NextResponse.json(
      { error: 'Email service configuration missing. Please verify your environment setups.' }, 
      { status: 500 }
    );
  }
  ...
  try {
    ...
    // 2. Strict Input validation matching frontend requirements
    if (!firstName || !lastName || !email) {
      console.warn("⚠️ Validation Failed: Required text values are missing", { firstName, lastName, email });
      return NextResponse.json({ error: 'Required fields (First Name, Last Name, and Email) are missing.' }, { status: 400 });
    }

    // Direct destination for the primary lead notice
    const businessOwnerEmail = process.env.OWNER_EMAIL || 'hello@northandknot.co.ke';
    console.log(`🚀 Attempting dispatch to team hub (${businessOwnerEmail})...`);
    
    // EMAIL ONE: Core Notification sent straight to the Business Owner
    ...
    if (ownerEmailPayload.error) {
      console.error("❌ Resend API rejected owner notification delivery:", ownerEmailPayload.error);
      return NextResponse.json({ error: 'Resend delivery failed', details: ownerEmailPayload.error }, { status: 400 });
    }

    console.log("✅ Lead notification successfully processed by Resend. Message ID:", ownerEmailPayload.data?.id);

    // EMAIL TWO: Safe-Guarded Client Auto-Receipt
    // To prevent unverified Resend sandbox limitations from completely breaking your API handler, 
    // we wrap this block inside an independent try/catch block.
    console.log(`📬 Attempting client confirmation bounce back to: ${email}...`);
    try {
      const clientEmailPayload = await resend.emails.send({
        from: 'North & Knot <onboarding@resend.dev>',
        to: email, 
      ...
      if (clientEmailPayload.error) {
        console.warn("⚠️ Client auto-receipt was blocked by Resend (This is normal behavior if you are in sandbox mode using an unverified domain):", clientEmailPayload.error);
      } else {
        console.log("✅ Client receipt accepted by Resend. ID:", clientEmailPayload.data?.id);
      }
    } catch (clientEmailErr) {
      console.warn("⚠️ Background catch caught a fallback failure on the client receipt module:", clientEmailErr);
    }

    return NextResponse.json({ success: true });
  } catch (err: any) {
    console.error('💥 FATAL REQ EXECUTION EXCEPTION:', err);
    return NextResponse.json({ error: 'Internal Server Error Encountered', details: err.message }, { status: 500 });
  }
}